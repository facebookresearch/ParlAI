...
class Kvmemnn(nn.Module):
    def __init__(self, opt, num_features, dict):
        ...
        for i in range(self.lins):
            self.lin.append(
                nn.Linear(opt['embeddingsize'], opt['embeddingsize'], bias=False))
        ####
            
    def forward(self, xs, mems, ys=None, cands=None, next_utts=None):
        scores = None
