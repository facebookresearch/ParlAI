...
class TransresnetMultimodalAgent(TransresnetAgent):
    ...
    def add_cmdline_args(argparser):
        ...
        argparser.add_argument('--personality-override', type=str, default=None,
                               help='for use in other tasks where no personality '
                               'is given. This will give the model a personality '
                               '(whichever is specifed).')
        argparser.add_argument('--personalities-path', type=str, default=None,
    ...
    def __init__(self, opt, shared=None):
        ...
        self.metrics = {k: {'hits@1/100': 0.0,
                            'loss': 0.0,
                            'num_samples': 0,
                            'med_rank': []}
                        for k in ['first_round',
                                  'second_round',
                                  'third_round+']}
        if shared is None:
    ...
    def eval_step(self, valid_obs, image_feats, personalities, dialogue_histories):
        ...
        if self.fixed_cands is not None:
            ...
            equality_list = [1 if chosen_responses[i][0] in labels[i]
                             else 0 for i in range(len(labels))]
            # calculate med ranks
            med_rank = []
    ...
    def batch_act(self, observations):
        ...
        personalities, dialogue_histories, dialogue_round = self.extract_texts(valid_obs)
        chosen_responses = None
        med_rank = None
        if is_training:
        ...
        self.update_metrics(loss, num_correct, len(valid_obs), dialogue_round,
                            med_rank)
        result = [{'text': 'No Response During Traiing', 'id': self.getID()}
    ...
    def update_metrics(self, loss, num_correct, num_samples, dialogue_round,
                       med_rank=None):
        """
        Update Metrics.
        ...
        for k, v in self.metrics.items():
            if v['num_samples'] > 0:
                m[k]['hits@1/100'] = round_sigfigs(
                    v['hits@1/100'] / v['num_samples'],
                    4)
                m[k]['loss'] = round_sigfigs(
                    v['loss'] / v['num_samples'],
                    4)
                if 'med_rank' in v:
