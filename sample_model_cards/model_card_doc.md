...
- - for classifiers, it should generate a folder `data/` that contains the relabeled data (by the default classifier `zoo:md_gender`) and another folder `quant_eval` that contains all the quantitative evaluation results 
...
By default, we use [`zoo:md_gender/model`](https://parl.ai/docs/zoo.html#mdgender-bert-ranker-classifier) to relabel the evaluation dataset, and then we re-evaluate the relabeled results separated by gender (not dataset). However, we can also specify other classifiers to use with (`--extra-args-path`)

To re-specify another model or multiple models, we would need to add pass a new file to `-exargs` (`--extra-args-path`). Here is the default one for quantitative analysis:
```
{
    "model_qargs": [
        {
...
```
The `model_qargs` is a list of arguments that help specify the classifier used for re-labeling of data. The fields `subgroup`, `model`, and `model_file` are required. The arguments `model` and `model_file` should be self-explanatory for those who have used [parlai](https://parl.ai/docs/cli_usage.html) before, and `subgroup` will later be the title for the heatmaps created. *Also, try to use not to use `:` in subgroup name since this may cause errors.*

*debugging tip 1: if things are failing at evaluation stage for quantitative analysis evaluation stage, try cleaning up the `data` folder.* 
