...
class _AbstractTRATest(unittest.TestCase):
    ...
    def test_eval_inline(self):
        args = self._get_args().fork(eval_candidates='inline')
        valid, test = testing_utils.train_model(args)
        threshold = self._get_threshold()
    ...
    def test_eval_fixed(self):
        ...
        # candidates
        teacher = CandidateTeacher(Opt(datatype='train'))
        ...
        with testing_utils.tempdir() as tmpdir:
            ...
            args = args.fork(
                fixed_candidates_path=tmp_cands_file,
                encode_candidate_vecs=False,  # don't encode before training
    ...
    def test_eval_vocab(self):
        args = self._get_args().fork(
            eval_candidates='vocab', encode_candidate_vecs=True
        )
        valid, test = testing_utils.train_model(args)
...
class TestTransformerRanker(_AbstractTRATest):
    def _get_args(self):
        ...
        return args.fork(model='transformer/ranker', n_layers=1, n_heads=4, ffn_size=32)


class TestMemNN(_AbstractTRATest):
    def _get_args(self):
        return super()._get_args().fork(model='memnn')
...
class TestPolyRanker(_AbstractTRATest):
    def _get_args(self):
        return (
            super()
            ._get_args()
            .fork(model='transformer/polyencoder', n_layers=1, n_heads=4, ffn_size=32)
        )
    ...
    def test_eval_fixed_label_not_in_cands(self):
        ...
        args = self._get_args().fork(
            model='parlai.agents.transformer.polyencoder:IRFriendlyPolyencoderAgent',
            eval_candidates='fixed',
        )
        ...
        with testing_utils.tempdir() as tmpdir:
            ...
            args = args.fork(
                ...
                num_epochs=4,
                add_label_to_fixed_cands=False,
            )
            ...
            args = args.fork(add_label_to_fixed_cands=True)
