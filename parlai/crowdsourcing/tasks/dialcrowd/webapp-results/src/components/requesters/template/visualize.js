    ...
    for (const [i, ratio] of this.props.ratios.entries()) {
      ...
      segments.push(
        <Tooltip placement="bottom" title={text}>
          <div style={{...barStyle, width, backgroundColor}}>
          </div>
        </Tooltip>
      );
      ...
      if (this.props.values !== undefined) {
        if (this.props.values[i].split(' ') > 3) {
          legendText = this.props.values[i].split(' ').splice(0, 3).join(' ') + ' ... ';
        } else {
    ...
    return (
      <div>
        ...
        <div>
          {legends.map(
            legend => (
              <span> <span style={{color: legend.color}}>⬤</span> {legend.text} &nbsp; </span>
            )
          )}
        </div>
      </div>
...
class ConfusionMatrix extends Component {
  ...
  render () {
    const [seriesMatrix, seriesLabel] = this.makeMatrix(
      this.props.annotations, this.props.options
    );
    return (
      <XYPlot
        colorType="literal"
  ...
  makeMatrix (annotations, options) {
    ...
    let matrix = Array(options.length).fill(0).map(
      i => Array(options.length).fill(0)
    );
    let sum = 0;
