...
export default function valid_utterance(text, search_results, selected_results, isOnboarding, taskConfig) {
  const bWords = taskConfig.bannedWords;
  const lowered_text = text.toLowerCase();
  return !(is_too_short(lowered_text, isOnboarding) ||
    ...
    needs_more_overlap_with_selected(lowered_text, search_results, selected_results));
}
...
function looks_like_greetings(text, isOnboarding) {
  ...
  if (GREETING_FAREWELL_WORDS.includes(first_word)) {
    alert("Your message looks like a greeting or farewell. Please try again and use more engaging messages.");
    return true;
  }
  return false;
}
...
function has_did_you_know(text) {
  if (text.includes("did you know") || text.includes("did u know")) {
    alert("Please try to be more engaging, and not use the phrase \'did you know\' :).");
    return true;
  }
  return false;
}
...
function has_banned_words(text, banned_words_list) {
  ...
  const banned_words = tokenized_text.filter((w) => banned_words_list.indexOf(w) !== -1)
  if (banned_words.length > 0) {
    const detected_banned_words = banned_words.join(', ');
    alert("We have detected the following offensive/banned language in your message: \"" +
...
function is_copy_pasted(text, docs) {
  if (!docs || docs.length === 0 || text.length < MIN_TEXT_LENGTH_TO_CHECK_COPY) {
    return false;
  }
  ...
  for (var doc_id in docs) {
    ...
    for (var sentence_id in document.content) {
      ...
      if (too_much_char_overlap(text, sentence)) {
        const q = "\"";
        alert("Your message has too much overlap with one of the candidates sentences. " +
...
function needs_more_overlap_with_selected(text, docs, selected) {
  ...
  for (var doc_id = 1; doc_id < selected.length; doc_id++) {
    ...
    for (var sent_id = 0; sent_id < docSelection.length; sent_id++) {
      if (docSelection[sent_id] === false) {  // This sentence was not selected
        continue;
      }
      // "doc_id-1" because selection has an extra value at index 0 (nothing selected)
      const lower_selected_sentence = docs[doc_id - 1].content[sent_id].toLowerCase();
  ...
  if (MIN_OVERLAP_REQUIRED >= num_overlaps) {
    return !confirm("Are you sure you are using the right checked sentence? " +
