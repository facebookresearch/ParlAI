...
class SequenceGenerator(nn.Module):
    def __init__(
        ...
        if isinstance(models, EnsembleModel):
            ...
            self.model = torch.nn.Module(models)
        self.tgt_dict = tgt_dict
        self.pad = tgt_dict[tgt_dict.null_token]
        self.unk = tgt_dict[tgt_dict.unk_token]
        self.eos = tgt_dict[tgt_dict.eos_token] if eos is None else eos
    ...
    def reorder_encoder_out(self, encoder_states, indices):
        """
    ...
    def _generate(
        ...
        if self.match_source_len:
            ...
            max_len = min(
                ...
                1e6#self.model.max_decoder_positions() - 1,
            )
        assert (
        ...
        #encoder_outs = self.model[0].encoder(*self._encoder_input(batch))
        ...
        if "id" in batch and isinstance(batch["id"], Tensor):
            original_batch_idxs = batch["id"]
        else:
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            #lprobs, avg_attn_scores = self.model.decoder(tokens[:, : step + 1], encoder_outs, incremental_states)
...
class EnsembleModel(nn.Module):
    ...
    def max_decoder_positions(self):
        return 128 #return min([m.max_decoder_positions() for m in self.models])
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            if self.has_incremental_states():
                ...
                decoder_out, new_incr_state = model.decoder.forward(tokens, encoder_out, incremental_states[i])

            attn: Optional[Tensor] = None
            decoder_len = len(decoder_out)
            if decoder_len > 1 and decoder_out is not None:
                if isinstance(decoder_out, Tensor):
                    attn = decoder_out
                else:
            ...
            decoder_out_tuple = (
                decoder_out[:,-1, :].div_(temperature),
                None if decoder_len <= 1 else decoder_out,
            )
