...
from parlai.core.torch_agent import TorchAgent, Output, Batch as TABatch
...
"""
A `torch_agent.Batch` namedtuple with the following additional fields:
...
class MemnnAgent(TorchAgent):
    ...
    def _set_text_vec(self, obs, add_start, add_end, truncate):
        ...
        Differs from TorchAgent.batchify in that it includes memories."""
    ...
    def batchify(self, obs_batch, sort=False):
        tabatch = super().batchify(obs_batch, sort)
        reordered_observations = [obs_batch[i] for i in tabatch.valid_indices]

        # MEMORIES
        mems = None
        if any('memory_vecs' in ex for ex in reordered_observations):
            mems = [ex.get('memory_vecs', None) for ex in reordered_observations]
