...
class Mlb(nn.Module):
    def __init__(self, opt, vocab):
        ...
        if self.opt['use_bayesian']:
            self.rnn = BayesianGRU(
                620, self.opt['dim_q'], dropout=self.opt['dropout_st']
            )
        else:
            self.rnn = nn.GRU(
                input_size=620,
    ...
    def set_init_states(self):
        ...
        state_dict = make_emb_state_dict(
            dictionary, parameters, self.dict.ind2tok.values()
        )
...
class MlbAtt(Mlb):
    def __init__(self, opt, vocab, states):
        ...
        self.conv_v_att = nn.Conv2d(self.opt['dim_v'], self.opt['dim_att_h'], 1, 1)
        self.linear_q_att = nn.Linear(self.opt['dim_q'], self.opt['dim_att_h'])
        self.conv_att = nn.Conv2d(self.opt['dim_att_h'], self.opt['num_glimpses'], 1, 1)
        if self.opt['original_att']:
            self.linear_v_fusion = nn.Linear(
                self.opt['dim_v'] * self.opt['num_glimpses'], self.opt['dim_h']
            )
            self.linear_q_fusion = nn.Linear(self.opt['dim_q'], self.opt['dim_h'])
            self.linear_classif = nn.Linear(self.opt['dim_h'], self.num_classes)
        else:
            ...
            self.linear_q_fusion = nn.Linear(
                self.opt['dim_q'], self.opt['dim_h'] * self.opt['num_glimpses']
            )
            self.linear_classif = nn.Linear(
                self.opt['dim_h'] * self.opt['num_glimpses'], self.num_classes
            )
    ...
    def forward_attention(self, input_v, x_q_vec):
        ...
        x_v = input_v
        x_v = F.dropout(x_v, p=self.opt['dropout_att_v'], training=self.training)
        ...
        x_q = F.dropout(x_q_vec, p=self.opt['dropout_att_q'], training=self.training)
        ...
        x_att = F.dropout(x_att, p=self.opt['dropout_att_mm'], training=self.training)
        ...
        x_att = x_att.view(batch_size, self.opt['num_glimpses'], width * height)
    ...
    def forward_glimpses(self, list_v_att, x_q_vec):
        ...
        if self.opt['original_att']:
            ...
            x_v = F.dropout(x_v, p=self.opt['dropout_v'], training=self.training)
            x_v = self.linear_v_fusion(x_v)
            x_v = getattr(F, self.opt['activation_v'])(x_v)
        else:
            for glimpse_id, x_v_att in enumerate(list_v_att):
                x_v = F.dropout(
                    x_v_att, p=self.opt['dropout_v'], training=self.training
                )
        ...
        # Process question
        x_q = F.dropout(x_q_vec, p=self.opt['dropout_q'], training=self.training)
