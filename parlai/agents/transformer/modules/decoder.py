...
class TransformerDecoderOnlyLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        new_incr_state = {'self_attn': final_self_attn_incr_state}
        return x, new_incr_state
...
class TransformerDecoderOnly(nn.Module):
    ...
    def __init__(
        ...
        embedding: nn.Embedding,
        n_positions: Optional[int] = None,
        dictionary: DictionaryAgent = None,
        **kwargs,
    ):
        super().__init__()
        self.opt = opt
    ...
    def forward_layers(
        ...
        **kwargs,
        ...
        new_incr_state = {}
        if getattr(self.layers, 'is_model_parallel', False):
            tensor, new_incr_state = self._apply_model_parallel(tensor, incr_state)
        else:
            for idx, layer in enumerate(self.layers):
                tensor, new_incr_state[idx] = layer(
                    x=tensor, incr_state=incr_state.get(idx), **kwargs
                )
    ...
    def _apply_model_parallel(self, tensor, incr_state):
        """
        Pipeline application of model parallelism.
        """
        chunks = PipelineHelper.split((tensor, incr_state))
        work_items = PipelineHelper.schedule_work_items(self.layers, chunks)

        new_incr_state = {i: [] for i, _ in enumerate(self.layers)}

        for chunk_idx, layer_nos, next_device in work_items:
            s_tensor, s_incr_state = chunks[chunk_idx]
            for layer_no in layer_nos:
                s_tensor, nis = self.layers[layer_no](
                    x=s_tensor, incr_state=s_incr_state.get(layer_no)
                )
                new_incr_state[layer_no].append(nis)
            # don't move incr state, it's always on the correct device
