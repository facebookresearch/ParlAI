...
class Seq2seqAgent(TorchAgent):
    ...
    @staticmethod
    def beam_search(model, batch, beam_size, start=1, end=2, pad=0, min_length=3, min_n_best=5, max_ts=40):
        ...
        beams = [Beam(beam_size, min_length=min_length, min_n_best=min_n_best, cuda=current_device) for i in range(batch_size)]
    ...
    def eval_step(self, batch):
        ...
        if self.search['mode'] == 'greedy':
            ...
            out = Seq2seqAgent.beam_search(self.model, batch, self.beam_size, start=self.START_IDX, end=self.END_IDX, pad=self.NULL_IDX, min_length=self.beam_min_length, min_n_best=self.beam_min_n_best)
