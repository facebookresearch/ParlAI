...
# 2 GPU machines: batchsize 32 num-epoch 0.1. 25% accuracy in 5min (+ eval)
# 8 GPU machines: batchsize 128 num-epoch 3. 79% accuracy in 40mn
...
# 2 GPU machines: batchsize 2 num-epoch 0.01 (!!!!!) 65% accuracy in 5min (+ eval)
# 8 GPU machines: batchsize 8 num-epoch 3. 85.2% accuracy in 8 hours
python -u parlai/scripts/train_model.py -pyt convai2 \
  ...
  --num-samples 131800 -veps 0.001 -vme 40 \
  --bert-adam false --bert-pad-left true
# Should yield
