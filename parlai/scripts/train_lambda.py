...
"""Modified from parlai/scripts/eval_model.py:
Basic example which iterates through train examples and fits lambda with 
the given pre-trained models.
...
def setup_args(parser=None):
    if parser is None:
        parser = ParlaiParser(True, True, 'Train lambda for a model')
    ...
    parser.set_defaults(datatype='valid') # OAD
    return parser
...
def train_lambda(opt, printargs=None, print_parser=None):
    """Trains lambda for pre-fit model components on training data.
...
if __name__ == '__main__':
    parser = setup_args()
    train_lambda(parser.parse_args(print_args=False), print_parser=parser)
