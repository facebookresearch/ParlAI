...
def get_dialog_data(datatype):
    """
    Load data from the checkpoint
    """
    dt = datatype.split(':')[0]
    fle = f'/checkpoint/parlai/tasks/gender_multiclass/opensubtitles/{dt}.txt.filtered'
...
def get_gender(name_str, dct, threshold=0.7):
    ...
    if name not in dct:
        if DEBUG:
            print(f'{name} not contained in dict.')
...
class OpensubtitlesTeacher(FixedDialogTeacher):
    ...
    NOTE: this is a single turn teacher, but we could make use of a
    better multi-turn teacher.
    """
