...
def _path(opt):
    ...
    if opt.get('light_unseen_test', False) == True or opt.get('light_unseen_test', False) == 'True':
        if dt == 'test':
            dt = 'test_unseen'
        else:
    ...
    return os.path.join(opt['datapath'], 'light_dialogue', fpath[:-1],
...
class DefaultTeacher(ParlAIDialogTeacher):
    ...
    def add_cmdline_args(argparser):
        ...
        agent.add_argument('--light_use_repeat', type=str, default='none',
        ...
        agent.add_argument('--light_use_persona', type=str, default='self',
                           choices = ['partner', 'self', 'all', 'none'])
        agent.add_argument('--light_use_objects', type='bool', default=True)
        agent.add_argument('--light_use_emote', type=str, default='all', choices = ['partner', 'self', 'all', 'none'])
        agent.add_argument('--light_use_speech', type=str, default='all', choices = ['partner', 'self', 'all', 'none'])
        agent.add_argument('--light_use_action', type=str, default='all', choices = ['partner', 'self', 'all', 'none'])
        agent.add_argument('--light_use_current_self_output', type=str, default="all",
                           choices = ['none', 'all', 'all_filtered', 'all_filtered_remove'])
        agent.add_argument('--light_label_type', type=str,
                           default='speech',
