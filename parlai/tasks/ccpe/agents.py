    ...
    def _setup_data(self):
        ...
        for ep in range(len(json_data)):
            ...
            for i, utterance in enumerate(json_data[ep]['utterances']):
                entry = []
                cnt = 0
                if i > 0 and json_data[ep]['utterances'][i-1]['speaker'] == "USER" and utterance['speaker'] == "ASSISTANT":
                    entry.append(cnt)
                    cnt +=1
                    entry.append(json_data[ep]['utterances'][i-1]['text'])
                    entry.append(utterance['text'])
                    entry.append(json_data[ep]['utterances'][i-1]['segments'] if 'segments' in json_data[ep]['utterances'][i-1] else [])
                    entry.append(json_data[ep]['utterances'][i]['segments'] if 'segments' in json_data[ep]['utterances'][i] else [])
                    entry.append(False)
                    currEp.append(entry)
