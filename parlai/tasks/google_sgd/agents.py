...
class GoogleSGDParser(tod_agents_and_teachers.TodStructuredDataParser):
    @classmethod
    def add_cmdline_args(
        ...
        parser.add_argument(
            ...
            default=-1,
            type=int,
            help="Number of dialogues (episodes) to keep. Where -1 means all",
        )
        parser.add_argument(
            "--n-shot-seed",
            default=None,
            type=int,
            help="Random seed for the n-shot subset",
        )
        parser.add_argument(
    ...
    def setup_episodes(self, fold):
        ...
        if self.fold != "test" and self.opt["n_shot"] > 0:
            # randomly sample, but perserve ordering so higher n-shot is a
            # strict superset
            rng = random.Random(self.opt["n_shot_seed"])
            rng.shuffle(result)
            result = result[: self.opt["n_shot"]]
        return result
...
class SingleGoalAgent(GoogleSGDParser, tod_agents_and_teachers.TodSingleGoalAgent):
    pass


class GoalAgent(GoogleSGDParser, tod_agents_and_teachers.TodGoalAgent):
    pass


class ApiSchemaAgent(GoogleSGDParser, tod_agents_and_teachers.TodApiSchemaAgent):
    pass


class UserUttAgent(GoogleSGDParser, tod_agents_and_teachers.TodUserUttAgent):
    pass
