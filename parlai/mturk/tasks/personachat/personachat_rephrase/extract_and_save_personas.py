...
def extract_and_save_from_pkl(opt, root):
    agent = RepeatLabelAgent(opt)
    world = create_task(opt, agent)
    teacher = world.agents[0]
    ...
    for filename in os.listdir(root):
        if 'incomplete' not in str(filename) and 'sandbox' not in str(filename):
            persona_dict = pickle.load(open(root+filename, 'rb'))
            new_list = []
            if len(persona_dict['persona']) >= 4:
                # edit personas
                for persona in persona_dict['persona']:
                    new_list.append(' '.join(persona.split()))
                personas.append(new_list)
...
def main(opt):
    ...
    opt['numthreads'] = 1
    opt['batchsize'] = 1
    extract_and_save_from_pkl(opt, '/private/home/edinan/ParlAI/data/personachat_collect_personas/personas/')
