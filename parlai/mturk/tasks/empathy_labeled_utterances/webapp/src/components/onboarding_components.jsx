...
function OnboardingUtterance({ annotationBuckets, annotationQuestion, turnIdx, text }) {
    ...
    return (
        <div className={`alert ${turnIdx % 2 == 0 ? "alert-info" : "alert-warning"}`} style={{ float: `${turnIdx % 2 == 0 ? "right" : "left"}`, display: 'table' }}>
            <span className="onboarding-text"><b>{turnIdx % 2 == 0 ? 'SPEAKER' : 'LISTENER'}:</b> {text}
                <ErrorBoundary>
                    {extraElements}
                </ErrorBoundary>
            </span>
    ...
    return (
        <div id="onboarding-main-pane">
            <OnboardingDirections>
                ...
                <div>
                    To first learn about the labeling task, please evaluate the LISTENER in the conversation below, choosing the correct checkboxes.</div>
            </OnboardingDirections>
