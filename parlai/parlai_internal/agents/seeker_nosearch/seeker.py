...
class SeekerAgent(ModularAgentMixin):
    ...
    def batch_act(self, observations: List[Dict[str, Message]], prefixes = None, chirpy_payload = None) -> List[Message]:
        """
        ...
        :param prefixes:
            batchsize-length list of prefixes of response

        :param chirpy customized input:
            None -> run krm module
            [{}] -> empty krm dictionary
            [{"chirpy_knowledge":"TOPIC"} ... ] -> krm topic override replacement
            ...
            if len(chirpy_payload) > 0 and "chirpy_knowledge" in chirpy_payload[0]:
                for i in range(len(batch_reply_krm)):
                    if chirpy_payload[i]["chirpy_knowledge"] != "":
                        batch_reply_krm[i]["chirpy_knowledge"] = chirpy_payload[i]["chirpy_knowledge"]
                        batch_reply_krm[i]["text"] = chirpy_payload[i]["chirpy_knowledge"]
                        batch_reply_krm[i]["source"] = "topic"
        else:
            batch_reply_krm = self.batch_act_krm(
                observations, knowledge_agent_observations, search_indices
            )
            for i in range(len(batch_reply_krm)):
                batch_reply_krm[i]["source"] = "krm"
        ...
        for sdm, sqm, krm, drm in zip(
            ...
            drm.force_set('krm_text_source', krm.get('source', "None"))

        return batch_reply_drm

    def act(self, prefix = None, chirpy_payload = None):
        """
        Call batch_act with the singleton batch.
        """
        response = self.batch_act([self.observations], prefixes = [prefix], chirpy_payload = [chirpy_payload] if chirpy_payload is not None else None)
        response = response[0]
